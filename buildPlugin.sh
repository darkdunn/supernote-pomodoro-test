...
build_android_apk() {
    ...
    local gradle_user_home="${GRADLE_USER_HOME:-$project_root/.gradle-local}"
    local android_user_home="${ANDROID_USER_HOME:-$project_root/.android-local}"
    mkdir -p "$gradle_user_home" "$android_user_home"

    write_color_output "Running gradle task: buildCustomApkDebug..." "Blue"
    write_color_output "Using GRADLE_USER_HOME: $gradle_user_home" "Blue"
